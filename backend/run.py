...
if __name__ == "__main__":
    ...
    if use_legacy:
        print(f"Running in LEGACY MODE (local storage, no auth)")
    else:
        if demo_mode:
            print(f"DEMO MODE enabled - Using in-memory storage")
            print(f"   OTP Code for all emails: 123456")
        else:
            print(f"CLOUD MODE - Using Supabase")
    
    uvicorn.run(module, host="0.0.0.0", port=int(os.environ.get("PORT", 8000)),reload=False)
